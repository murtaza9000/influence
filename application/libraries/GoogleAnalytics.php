...
class Googleanalytics
{
    ...
    public function execute_per_profile(&$analytics, $profile, $facebook = null)
    {
        ...
        if (count($premiumResults) > 0) {
            foreach ($premiumResults as $result) {
                ...
                $totalAmount += $amount;
                $this->update_amount_in_map($name, $amount);
    ...
    public function update_amount_in_map($name, $amount)
    {
        if (!isset($this->amountMap[$name])) {
            $this->amountMap[$name] = $amount;
        } else {
            $this->amountMap[$name] += $amount;
        }
    }
    ...
    private function print_the_map(){
        foreach ($this->amountMap as $key => $value) {
            echo $key . ": " . $value . "\n";
        }
    }
    ...
    {
        //Loop through the map and update all the influencers
        //foreach($this->urlToAccountMap as $key => $value){
        foreach ($this->amountMap as $key => $value) {
            //If we have the inf in the DB get him otherwise insert him
            $inf = $this->CI->db->get_where('influencer', array('utm' => $key))->row();
            if (!$inf) {
                //die('found nothing');
                $data = array(
                    'utm' => $key
            ...
            //
            echo PHP_EOL . '[-] Processing UTM: ' . $key . PHP_EOL;
            //echo "Sessions: " . $sessions . PHP_EOL;
            echo "Amount: " . $value . PHP_EOL;
            ...
            echo "Current Payment: " . $currentPayment . " Same Day:" . $sameDay . " Yesterday Peyment: " . $yesterdayPayment . " , Amount: " . $value . PHP_EOL;
            ...
            $this->update_influencer_amount($currentPayment, $sameDay, $yesterdayPayment, $value, $now, $now_time, $inf);
